...
import (
	...
	"github.com/Ahmad-mufied/learn-golang-restful-api/model/domain"
...
func setupTestDB() *sql.DB {
	db, err := sql.Open("mysql", "admin:password@tcp(localhost:3306)/learn_golang_restful_api_test?parseTime=true")
...
func TestDeleteCategoryFailed(t *testing.T) {
	...
	requestBody := strings.NewReader(`{"name": "Gadget"}`)
	request := httptest.NewRequest(http.MethodPut, "http://localhost:3000/api/categories/404", requestBody)
	request.Header.Add("Content-Type", "application/json")
	request.Header.Add("X-API-Key", "RAHASIA")
